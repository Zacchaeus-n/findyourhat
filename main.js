...
class Field {
  ...
  playGame() {
    ...
    while (currentlyPlaying) {
      moveSelection = prompt(
        "Which way would you like to move? (Up = 'U' Down = 'D' Left = 'L' Right = 'R'"
      );
      ...
      } else if (moveSelection.toUpperCase() === "D" && y !== inputHgt - 1) {
        y += 1;
      } else if (moveSelection.toUpperCase() === "D" && y === inputHgt - 1) {
  ...
  static generateField(height, width, percentage) {
    // Helper Functions
    const countFieldCharacters = (arrOfHolesAndFieldChars) => {
      let fieldCharCount = 0;
      for (let j = 0; j < arrOfHolesAndFieldChars.length; j++) {
        if (arrOfHolesAndFieldChars[j] === fieldCharacter) {
          fieldCharCount++;
        }
      }
      return fieldCharCount;
    };
    // Create a field with only holes and field characters
    let outputField = [];
    let randomInt;
    let currentArray = [];
    let rowCount = 0;
    ...
    const randomHatHgt = Math.floor(Math.random() * height);
    const randomHatWidth = Math.floor(Math.random() * width);
    outputField[0][0] = pathCharacter;
    outputField[0][1] = fieldCharacter;
    outputField[randomHatHgt][randomHatWidth] = hat;
    return outputField;
  }
}

// Prompts at the beginning of the game to generate field
let inputHgt = prompt("How tall would you like your game field to be?");
let inputWidth = prompt("How wide?");
let inputPercent = prompt("Percentage of pathway vs. holes?");
